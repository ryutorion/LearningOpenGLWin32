...
bool OnCreate(HWND hWnd)
{
    ...
    {
        cout << "Failed to get glGenBuffers address!" << endl;
        return false; 
    }

    glDeleteBuffers = reinterpret_cast<decltype(glDeleteBuffers)>(
        wglGetProcAddress("glDeleteBuffers")
    );
    if(glDeleteBuffers == nullptr)
    {
        cout << "Failed to get glDeleteBuffers address!" << endl;
        return false;
    }

    glBindBuffer = reinterpret_cast<decltype(glBindBuffer)>(
        wglGetProcAddress("glBindBuffer")
    );
    if(glBindBuffer == nullptr)
    {
        cout << "Failed to get glBindBuffers address!" << endl;
        return false;
    }

    glBufferData = reinterpret_cast<decltype(glBufferData)>(
        wglGetProcAddress("glBufferData")
    );
    if(glBufferData == nullptr)
    {
        cout << "Failed to get glBufferData address!" << endl;
        return false;
    }
...
void OnDraw(HWND hWnd)
{
    ...
    int components = 2;
    glVertexPointer(components, GL_FLOAT, 0, 0);
